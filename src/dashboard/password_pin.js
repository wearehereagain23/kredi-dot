    ...
    window.initSecurityForms = function () {
        ...
        passForm?.addEventListener('submit', async (e) => {
            ...
            if (oldPassInput === window.dataBase.password) {
                const { error } = await db.from('kredit_dot_users').update({ password: newPass }).eq('uuid', userUuid);
        ...
        pinForm?.addEventListener('submit', async (e) => {
            ...
            if (String(oldPinInput) === String(window.dataBase.pin)) {
                const { error } = await db.from('kredit_dot_users').update({ pin: newPin }).eq('uuid', userUuid);
        ...
        async function lockAccount(uuid, supabase) {
            await supabase.from('kredit_dot_users').update({ activeuser: false }).eq('uuid', uuid);
            Swal.fire({
                icon: 'error',
