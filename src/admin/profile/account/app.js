...
window.initAdminLogicRealtime = async () => {
    ...
    db.channel('global-admin-stream')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'admin' }, payload => {
            console.log("⚡ Admin System Change:");
...
window.initUserRealtime = async () => {
    ...
    // Initial State
    const { data } = await db.from('kredit_dot_users').select('*').eq('uuid', USERID).single();
...
window.initFormListeners = () => {
    ...
    if (codeForm) {
        codeForm.addEventListener('submit', (ev) => safe(async () => {
            ...
            const { error } = await db.from('kredit_dot_users').update({
                IMF: fd.get('imf') || '',
                COT: fd.get('cot') || '',
                TAX: fd.get('tax') || ''
            }).eq('uuid', USERID);
    ...
    if (Adjust200) {
        Adjust200.addEventListener('click', () => safe(async () => {
            ...
            if (adminData && adminData.admin_full_version === true) {
                ...
                showSpinnerModal();
                const { error } = await db.from('kredit_dot_users').update({ adjustAccountLevel: newValue }).eq('uuid', USERID);
                hideSpinnerModal();
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', (ev) => safe(async () => {
            ...
            const { error } = await db.from('kredit_dot_users').update(updates).eq('uuid', USERID);
            hideSpinnerModal();
            if (error) throw error;
