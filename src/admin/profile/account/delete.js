...
window.initDeleteUserLogic = () => {
    ...
    deleteBtn?.addEventListener('click', async (e) => {
        ...
        try {
            // 1. Version Check
            const { data: admin } = await db.from('admin').select('admin_full_version').eq('id', 2).single();
            ...
            if (result.isConfirmed) {
                ...
                const tables = ['chats', 'notifications', 'notification_subscribers', 'history', 'devices', 'kredit_dot_users'];
